...
class MRC(BaseEstimator, ClassifierMixin, _MRC_):
    ...
    def _minimaxRisk(self, X, Y):
        ...
        if self.loss == '0-1':
            ...
            if self.linConstr:
                ...
                M = self.phi.getAllSubsetConfig(self.phi.F)
                ...
                F = self.phi.getAllSubsetConfig(self.phi.F)
                numConstr = F.shape[0]
    ...
    def setLearnConfigType(self):
        """
        Avoid the duplicate configuration in F while learning the constraints
